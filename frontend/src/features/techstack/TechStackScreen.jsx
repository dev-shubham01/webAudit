...
export default function TechStackScreen({ report }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Tech Detection</h1>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {Object.entries(categoryCounts)
          .slice(0, 4)
          .map(([cat, count]) => (
            <Card key={cat} shadow className="border-border bg-card">
              <CardContent className="p-4">
                <div className="mb-1 text-xs font-bold uppercase text-muted-foreground">{cat}</div>
      ...
      {chartData.length > 0 ? (
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-8 text-center text-sm text-muted-foreground">No technology data. Run a crawl first.</CardContent>
        </Card>
      )}

      {techs.length > 0 && (
        <div>
          <h2 className="mb-4 text-xl font-bold text-foreground">Technology Breakdown</h2>
          <Card className="border-border bg-card">
                ...
                <tbody>
                  {techs.map((t) => (
                    <tr key={t.name} className="border-b border-border/50">
                      <td className="px-6 py-2 font-medium text-foreground">{t.name}</td>
                      <td className="px-4 py-2 text-xs text-muted-foreground">{categorizeTech(t.name)}</td>
                      <td className="px-4 py-2 text-right font-mono text-muted-foreground">{t.count.toLocaleString()}</td>
