...
export default function ThresholdBar({ metricKey, value }) {
  ...
  if (!t || value == null) {
    return (
      <div className="flex items-center justify-between px-5 py-4">
        <span className="text-sm text-[#E2E8F0]">{t?.label || metricKey}</span>
  ...
  return (
    <div
      className="relative flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[#1E293B]"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      ref={barRef}
    >
      <span className="w-44 shrink-0 text-sm text-[#E2E8F0]">{t.label}</span>
      <div className="flex flex-1 items-center gap-3">
        <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-[#334155]">
          <div
      ...
      {hovered && (
        <div className="pointer-events-none absolute bottom-full left-0 z-50 mb-2 ml-4 w-72 rounded-xl border border-[#334155] bg-[#1E293B] p-3 shadow-2xl">
          <div className="mb-1 text-sm font-semibold text-[#E2E8F0]">{t.label}</div>
          <p className="mb-2 text-xs text-[#94A3B8]">{t.desc}</p>
          <div className="flex gap-4 text-xs">
            <span>
              <span className="text-[#94A3B8]">Value:</span> <span className={textColor}>{formatMetric(metricKey, v)}</span>
            </span>
            <span>
            ...
            <span>
              <span className="text-[#94A3B8]">Warn:</span>{" "}
